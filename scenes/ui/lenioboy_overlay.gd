...
onready var shader_rect_parent = $Shaders
...
func reset_crt_vhs_shader() -> void:
	if crt_vhs_material is ShaderMaterial:
		for param_name in crt_vhs_defaults.keys():
			crt_vhs_material.set_shader_param(param_name, crt_vhs_defaults[param_name])


func set_crt_vhs_level(crt_level: int) -> void:
	match crt_level:
		6:
			crt_vhs_material.set_shader_param("static_noise_intensity", 0.5)
		7:
			crt_vhs_material.set_shader_param("static_noise_intensity", 0.6)
...
func reset_audio_effects() -> void:
	...
	var num_effects = AudioServer.get_bus_effect_count(0)
