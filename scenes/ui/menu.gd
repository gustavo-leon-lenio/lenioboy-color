...
@onready
var rich_text_label: RichTextLabel = $CanvasLayer/MainMenu/MarginContainer/VBoxContainer/HBoxContainer2/RichTextLabel
@onready
var play_button: Button = $CanvasLayer/MainMenu/MarginContainer/VBoxContainer/HBoxContainer/Play
...
func _ready() -> void:
	...
	ok_button = name_input_dialog.get_ok_button()
	Global.load_player_data()
	line_edit.text = Global.player_data.name
	toggle_ok_button(line_edit.text)
	rich_text_label.text = bbcode_wrapper_effects(Global.player_data.name)


func is_alphanumeric_and_not_empty(text: String) -> bool:
	if text.is_empty():
		return false
